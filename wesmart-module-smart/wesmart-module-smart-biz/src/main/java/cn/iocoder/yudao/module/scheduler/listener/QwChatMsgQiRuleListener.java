...
//            channel.basicNack(message.getMessageProperties().getDeliveryTag(), true, true);
    ...
    private Boolean isExternal(String externalUserId) {
        int count = (int)weCustomerService.count(new LambdaQueryWrapper<WeCustomer>().eq(WeCustomer::getExternalUserid, externalUserId).eq(WeCustomer::getDelFlag, 0));
        if (count > 0) {
            return true;
        }
        return false;
    ...
    private Boolean isGroupExternal(String externalUserId, String roomId) {
        long count = weGroupMemberService.count(new LambdaQueryWrapper<WeGroupMember>()
