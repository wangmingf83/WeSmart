...
import cn.iocoder.yudao.module.common.constant.MessageConstants;
...
import cn.iocoder.yudao.module.common.enums.task.WeTasksTitleEnum;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.leads.record.query.WeLeadsFollowRecordCooperateUserRequest;
import cn.iocoder.yudao.module.smart.dal.dataobject.leads.sea.entity.WeLeadsSea;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.sop.dto.WeSopPushTaskDto;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.task.entity.WeTasks;
...
import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeLeadsMapper;
import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeLeadsSeaMapper;
import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeSopExecuteTargetAttachmentsMapper;
import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeTasksMapper;
...
public class WeTasksServiceImpl extends ServiceImpl<WeTasksMapper, WeTasks> implements IWeTasksService {
    ...
    public List<WeTasksVO> myList() {
        ...
        queryWrapper.eq(WeTasks::getUserId, ScrmSecurityUtils.getUserId());
    ...
    public void addLeadsLongTimeNotFollowUp(Long leadsId, String name, Long userId, String weUserId, Long seaId) {
        ...
        String leadsDetailUrl = GuituAiConfig.getLeadsDetailUrl();
    ...
    public void addCustomerSop(WeTasksRequest request) {
        ...
        String REDIRECT_URI = URLEncoder.encode(String.format("%s?corpId=%s&agentId=%s&type=%s&isExpiringSoon=%s",
                GuituAiConfig.getCustomerSopRedirectUrl(), weCorpAccount.getCorpId(), weCorpAccount.getAgentId(), MessageNoticeType.CUSTOMER_SOP.getType(), false));

        //员工信息
        AdminUserAllDTO info = scrmSysUserClient.findSysUserByWeUserId(request.getWeUserId());
    ...
    public void addGroupSop(WeTasksRequest request) {
        ...
        WeCorpAccount weCorpAccount = weCorpAccountService.getCorpAccountByCorpId(null);
        String REDIRECT_URI = URLEncoder.encode(String.format("%s?corpId=%s&agentId=%s&type=%s&isExpiringSoon=%s",
                GuituAiConfig.getGroupSopRedirectUrl(), weCorpAccount.getCorpId(), weCorpAccount.getAgentId(), MessageNoticeType.GROUP_SOP.getType(), false));
    ...
    public void handlerAppointItemWaitFollowUp(WeTasksRequest request) {
        String h5Domain = GuituAiConfig.getH5Domain();
        String leadsCovenantWaitFollowUpUrl = GuituAiConfig.getLeadsCovenantWaitFollowUpUrl();
    ...
    public void handlerGroupAddByLabel(WeTasksRequest request) {
        ...
        List<AdminUserAllDTO> result = scrmSysUserClient.getUserListByWeUserIds(query.getWeUserIds());
//        if (result.getCode() != HttpStatus.SUCCESS) {
        if (result == null) {
//            throw new ServiceException("获取员工数据异常：" + result.getMsg());
            throw new ServiceException("获取员工数据异常：");
        }
        List<AdminUserAllDTO> data = result;
        WeCorpAccount weCorpAccount = weCorpAccountService.getCorpAccountByCorpId(null);
        ...
        for (AdminUserAllDTO datum : data) {
