...
import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeQiRuleMapper;
...
public class WeQiRuleServiceImpl extends ServiceImpl<WeQiRuleMapper, WeQiRule> implements IWeQiRuleService {
    ...
    public WeQiRuleDetailVo getQiRuleDetail(Long id) {
        ...
        if (CollectionUtil.isNotEmpty(userIds)) {
            ...
            List<AdminUserAllDTO> userList = scrmSysUserClient.getUserListByWeUserIds(new ArrayList<>(userIds));
            if (CollectionUtil.isNotEmpty(userList)) {
                userMap = userList.stream().collect(Collectors.toMap(AdminUserAllDTO::getWeUserId, AdminUserAllDTO::getUsername, (key1, key2) -> key2));
            }
        }
...
//            }
        ...
        if (CollectionUtil.isNotEmpty(tableVoList)) {
            ...
            SysUserQuery userQuery = new SysUserQuery();
            userQuery.setWeUserIds(new ArrayList<>(userIds));
            List<AdminUserAllDTO> userVoList = scrmSysUserClient.getUserListByWeUserIds(new ArrayList<>(userIds));
            Map<String, AdminUserAllDTO> userIdMap = new HashMap<>();
            if (CollectionUtil.isNotEmpty(userVoList)) {
                userIdMap = userVoList.stream().collect(Collectors.toMap(AdminUserAllDTO::getWeUserId, Function.identity(), (key1, key2) -> key2));
            }
            for (WeQiRuleStatisticsTableVo tableVo : tableVoList) {
                if (userIdMap.containsKey(tableVo.getUserId())) {
                    AdminUserAllDTO sysUserVo = userIdMap.get(tableVo.getUserId());
...
//            }
        ...
        if (CollectionUtil.isNotEmpty(weeklyList)) {
            ...
            List<AdminUserAllDTO> userVoList = scrmSysUserClient.getUserListByWeUserIds(weUserIds);
            Map<String, String> userIdMap = new HashMap<>();
            if (CollectionUtil.isNotEmpty(userVoList)) {
                userIdMap = userVoList.stream().collect(Collectors.toMap(AdminUserAllDTO::getWeUserId, AdminUserAllDTO::getUsername, (key1, key2) -> key2));
            }
        ...
        if (CollectionUtil.isNotEmpty(weeklyDetailList)) {
            ...
            for (WeQiRuleWeeklyDetailListVo detailListVo : weeklyDetailList) {
                if (userIdMap.containsKey(detailListVo.getUserId())) {
                    AdminUserAllDTO sysUserVo = userIdMap.get(detailListVo.getUserId());
