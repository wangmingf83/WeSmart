...
import cn.iocoder.yudao.module.wecom.service.IQwMsgAuditService;
...
import cn.iocoder.yudao.module.common.utils.thread.WeMsgAuditThreadExecutor;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.callback.WeBackBaseVo;
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.query.msgaudit.WeMsgAuditQuery;
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.vo.msgaudit.WeMsgAuditVo;
...
public class WeMsgAuditTask {
        ...
        if (corpAccount != null) {
            ...
            if(StringUtils.isNotEmpty(corpAccount.getCorpId()) && StringUtils.isNotEmpty(corpAccount.getChatSecret()) && StringUtils.isNotEmpty(corpAccount.getFinancePrivateKey())){
                FinanceService financeService = new FinanceService(corpAccount.getCorpId(), corpAccount.getChatSecret(), corpAccount.getFinancePrivateKey(),
                        GuituAiConfig.getFincaceProxyConfig().getProxy(),GuituAiConfig.getFincaceProxyConfig().getPaswd());
    ...
    public void getPermitUserListHandle() {
        ...
        WeMsgAuditVo weMsgAuditVo = qwMsgAuditService.getPermitUserList(auditQuery);
        ...
        if (Objects.nonNull(weMsgAuditVo) && ObjectUtil.equal(0, weMsgAuditVo.getErrCode())) {
            ...
            scrmSysUserClient.updateUserChatStatus(userIds);
        }
    }
    ...
    public void checkSingleAgreeTaskHandle() {
        ...
        long total = weCustomerService.count(new QueryWrapper<WeCustomer>()
        ...
        // 是不是整除
        int surplus = (int)total % pageSize;
        if (surplus > 0) {
            pageNum = pageNum + 1;
        }
            ...
            WeMsgAuditThreadExecutor.getInstance().execute(() -> {
                try {
                    ...
                    if (CollectionUtil.isNotEmpty(externalUserIdAndUserIdList)) {
                        ...
                        WeMsgAuditVo weMsgAuditVo = qwMsgAuditService.checkSingleAgree(auditQuery);
        ...
        Optional.ofNullable(groupList).orElseGet(ArrayList::new).stream().map(WeGroup::getChatId).forEach(roomId ->{
            WeMsgAuditThreadExecutor.getInstance().execute(() ->{
                try {
                    ...
                    WeMsgAuditVo weMsgAuditVo = qwMsgAuditService.checkRoomAgree(auditQuery);
