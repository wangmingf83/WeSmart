...
import cn.iocoder.yudao.module.common.enums.moments.task.WeMomentsTaskSendTypEnum;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.moments.dto.*;
import cn.iocoder.yudao.module.smart.dal.dataobject.moments.entity.*;
...
import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeMomentsTaskMapper;
...
//                    }
                    List<AdminUserAllDTO> ajaxResult = scrmSysUserClient.getUserListByWeUserIds(senderList.stream().map(WeAddGroupMessageQuery.SenderInfo::getUserId).collect(Collectors.toList()));
                    if(null !=ajaxResult && CollectionUtil.isNotEmpty(ajaxResult)){
                        weMomentsUserService.addMomentsUser(task.getId(), ajaxResult);
                    }
    ...
    public WeMomentsTaskVO get(Long weMomentsTaskId) {
        ...
        if (weMomentsTask.getIsLwPush().equals(0)) {
            ...
            int count = (int)weMomentsCustomerService.count(customerWrapper);
        ...
        if (weMomentsTask.getStatus().equals(1)) {
            ...
            if (CollectionUtil.isNotEmpty(list)) {
                for (WeMomentsTaskRelation relation : list) {
                    //停止朋友圈
                    qwMomentsService.cancelMomentTask(MomentsCancelDTO.builder().moment_id(relation.getMomentId()).build());
                }
            }
    ...
    private void sendWeMoments(WeMomentsTask weMomentsTask, List<Long> materialIds) throws IOException {
        ...
        if (weMomentsTask.getSendType().equals(WeMomentsTaskSendTypEnum.ENTERPRISE_GROUP_SEND.getCode())){
            ...
            if(CollectionUtil.isNotEmpty(weMomentsUsers)){
                ...
                MomentsResultDto result = qwMomentsService.addMomentTask(dto);
//                if (result.getCode() == HttpStatus.SUCCESS) {
            ...
            if(CollectionUtil.isNotEmpty(estimateUsers)){
                ...
                for (List<String> list : split) {
                    ...
                    String url = StringUtils.format(GuituAiConfig.getMomentsUrl(), weMomentsTask.getId());
     ...
     * @return {@link List<AdminUserAllDTO>}
     * @author WangYX
     * @date 2023/06/07 15:44
     */
    public List<AdminUserAllDTO> getSysUser(WeMomentsTaskAddRequest request) {
    ...
    private void convertWeChatMaterial(List<Object> attachments, WeMaterial weMaterial) {
        ...
        if (StringUtils.isNotEmpty(media_id)) {
            MomentsParamDto.Link build = new MomentsParamDto.Link();
            //移动端素材详情地址
            String materialDetailUrl = GuituAiConfig.getMaterialDetailUrl();
    ...
    private MomentsParamDto buildMomentsParam(WeMomentsTaskAddRequest request) throws IOException {
        List<AdminUserAllDTO> sysUser = this.getSysUser(request);
        List<String> weUserIds = sysUser.stream().map(AdminUserAllDTO::getWeUserId).collect(Collectors.toList());
    ...
    public void getMomentPage(MomentsListDetailParamDto query){
        MomentsListDetailResultDto result = qwMomentsService.getMomentList(query);
        if (null != result) {
...
//        }
        ...
        wrapper.eq(WeMomentsEstimateUser::getWeUserId, ScrmSecurityUtils.getLoginUser().getWeUserId());
...
//        if (result.getCode() == HttpStatus.SUCCESS) {
        if (result != null) {
            MomentsListDetailResultDto data = result;
    ...
    public void jobIdToMomentsId(List<WeMomentsTaskRelation> relations) {
        ...
        if(CollectionUtil.isNotEmpty(relations)){
            relations.stream().forEach(k->{
                MomentsCreateResultDto momentTaskResult = qwMomentsService.getMomentTaskResult(k.getJobId());
                if (momentTaskResult != null) {
//                        if (momentTaskResult.getCode() == HttpStatus.SUCCESS) {
                            MomentsCreateResultDto data = momentTaskResult;
    ...
    private void syncAndSave(MomentsListDetailResultDto.Moment moment) {
        ...
        if(CollectionUtil.isNotEmpty(weMomentsTaskRelations)){
            ...
            if (BeanUtil.isNotEmpty(one)) {
                ...
                AdminUserAllDTO sysUser = null;
                AdminUserAllDTO info = scrmSysUserClient.findSysUserByWeUserId(creator);
//                if (info.getCode() == HttpStatus.SUCCESS) {
     ...
     */
    private void syncAddMomentsRelation(MomentsListDetailResultDto.Moment moment, AdminUserAllDTO sysUser) {
        //1.新增朋友圈
     ...
     */
    private Long syncAddMomentsTask(MomentsListDetailResultDto.Moment moment, AdminUserAllDTO sysUser) {
        ...
        if (BeanUtil.isNotEmpty(sysUser)) {
            ...
            task.setCreateById(sysUser.getId());
        }
