...
import cn.iocoder.yudao.module.common.constant.*;
...
import cn.iocoder.yudao.module.common.enums.*;
import cn.iocoder.yudao.module.common.enums.message.MessageTypeEnum;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.customer.vo.*;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.entity.customer.WeCustomerFollowInfoEntity;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.query.customer.*;
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.query.customer.tag.WeMarkTagQuery;
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.query.customer.transfer.WeTransferCustomerQuery;
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.vo.WeResultVo;
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.vo.customer.*;
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.vo.customer.transfer.WeTransferCustomerVo;

import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeCustomerMapper;
...
public class WeCustomerServiceImpl extends ServiceImpl<WeCustomerMapper, WeCustomer> implements IWeCustomerService {
    ...
    public TableDataInfo<List<WeCustomersVo>> findWeCustomerListByApp(WeCustomersQuery weCustomersQuery, PageDomain pageDomain) {
        ...
        if (weCustomersQuery.getDataScope()) {//这里指的全部数据,是角色的那些数据
            ...
            weCustomersQuery.setFirstUserId(
                    ScrmSecurityUtils.getLoginUser().getWeUserId()
            );
    ...
    public void synchWeCustomer() {
        ...
        WeFollowUserListVo followUserList = qwCustomerService.getFollowUserList(new WeBaseQuery());
...
//        SecurityContextHolder.setUserId(String.valueOf(loginUser.getSysUser().getUserId()));
        SecurityContextHolder.setUserId(String.valueOf(ScrmSecurityUtils.getUserId()));
//        SecurityContextHolder.setUserType(loginUser.getUserType());
    ...
    public void synchWeCustomerByAddIds(List<String> followUserIds) {
        if (CollectionUtil.isNotEmpty(followUserIds)) {
            ...
            Map<String, AdminUserAllDTO> currentTenantSysUser = scrmSysUserClient.findCurrentTenantSysUserMap();
...
//    }
    ...
    public void buildAllocateWecustomer(List<String> followUserIds) {
        ...
        // 批量更新 addUserLeave 字段
        this.update(
                new LambdaUpdateWrapper<WeCustomer>()
                        .set(WeCustomer::getAddUserLeave, 1)
                        .in(WeCustomer::getAddUserId, followUserIds)
        );
    }

    private void getByUser(List<String> followUser, String nextCursor, Map<String, AdminUserAllDTO> currentTenantSysUser) {

        WeBatchCustomerDetailVo weBatchCustomerDetails = qwCustomerService
                .getBatchCustomerDetail(new WeBatchCustomerQuery(followUser, nextCursor, 100));
    ...
    private void getBySingleUser(String externalUserid,String nextCursor,List<WeCustomerDetailVo> list){
        WeCustomerDetailVo weBatchCustomerDetails = qwCustomerService.getCustomerDetail(WeCustomerQuery.builder().external_userid(externalUserid)
                .cursor(nextCursor).build());

        if(null != weBatchCustomerDetails){
            if (WeErrorCodeEnum.ERROR_CODE_0.getErrorCode().equals(weBatchCustomerDetails.getErrCode())
    ...
    //客户同步业务处理,入库
    private void weFlowerCustomerHandle(List<WeCustomerDetailVo> details, Map<String, AdminUserAllDTO> currentTenantSysUser) {
        ...
        details.stream().forEach(k -> {
            ...
            if (null != followInfo && null != externalContact) {
                ...
                AdminUserAllDTO sysUser = currentTenantSysUser.get(followInfo.getUserId());

                if (null != sysUser) {
                    weCustomer.setCreateBy(sysUser.getUsername());
                    weCustomer.setCreateById(sysUser.getId());
                    weCustomer.setUpdateBy(sysUser.getUsername());
                    weCustomer.setUpdateById(sysUser.getId());
                }
                ...
                if (CollectionUtil.isNotEmpty(tagIds)) {
                    ...
                    tagIds.stream().forEach(tagId -> {
                        ...
                        if (null != sysUser) {
                            ...
                            weFlowerCustomerTagRel.setCreateById(sysUser.getId());
                            weFlowerCustomerTagRel.setUpdateBy(sysUser.getUsername());
                            weFlowerCustomerTagRel.setUpdateById(sysUser.getId());
                        }
...
//            }
    ...
    public void makeLabel(WeMakeCustomerTag weMakeCustomerTag) {
        ...
        if (CollectionUtil.isNotEmpty(addTag)) {
            ...
            addTag.stream().forEach(k -> {
                ...
                WeFlowerCustomerTagRel weFlowerCustomerTagRel = WeFlowerCustomerTagRel.builder()
                        ...
                        .build();
                weFlowerCustomerTagRel.setUpdateTime(new Date());
                weFlowerCustomerTagRel.setUpdateBy(String.valueOf(ScrmSecurityUtils.getUserId()));
        ...
        if (weMakeCustomerTag.getIsCompanyTag()) {//企业标签,同步企业微信客户端
            ...
            //发送消息客户标签同步企业微信端
            WeResultVo weResultVoAjaxResult = qwCustomerService.makeCustomerLabel(cutomerTagEdit);

            if (null != weResultVoAjaxResult && weResultVoAjaxResult.getErrMsg() != "") {
                log.error(weResultVoAjaxResult.getErrMsg());
    ...
    public WeCustomerDetailInfoVo findWeCustomerInfoByUserId(String externalUserid, String userId, Integer delFlag) {
        ...
        if (CollectionUtil.isNotEmpty(groups)) {
            List<WeCustomerAddGroupVo> commonGroup
                    = groups.stream().filter(e -> e.getCommonGroup().intValue()==0).collect(Collectors.toList());
    ...
    public void updateWeCustomerPortrait(WeCustomerPortraitVo weCustomerPortrait) {
        ...
        WeResultVo weResultVoAjaxResult = qwCustomerService.updateCustomerRemark(remarkQuery);
        if (null != weResultVoAjaxResult) {
            WeResultVo weResultVo = weResultVoAjaxResult;
    ...
    private void extentCustomer(WeCustomer weCustomer, WeOnTheJobCustomerQuery weOnTheJobCustomerQuery) throws WeComException {
        ...
        if (this.update(
            ...
            WeTransferCustomerVo jobExtendsCustomer = qwCustomerService.transferCustomer(
            ...
            if (null != jobExtendsCustomer) {
                WeTransferCustomerVo transferCustomerVo = jobExtendsCustomer;
    ...
    public void addCustomer(String externalUserId, String userId, String state) {
        ...
        WeCustomerDetailVo weCustomerDetail = qwCustomerService.getCustomerDetail(query);
        ...
        if (weCustomerDetail != null && weCustomerDetail.getExternalContact() != null) {
            ...
            if (CollectionUtil.isNotEmpty(followUserList)) {
                ...
                Map<String, AdminUserAllDTO> currentTenantSysUser = findCurrentTenantSysUser();

                if (CollectionUtil.isNotEmpty(currentTenantSysUser)) {
                    AdminUserAllDTO sysUser = currentTenantSysUser.get(followUserEntity.getUserId());
    ...
    @Override
    public Map<String, AdminUserAllDTO> findCurrentTenantSysUser() {
        Map<String, AdminUserAllDTO> sysUserMap = new HashMap<>();
        List<AdminUserAllDTO> sysUsers = scrmSysUserClient.findCurrentTenantSysUser();
        if (CollectionUtil.isNotEmpty(sysUsers)) {
            sysUserMap = sysUsers.stream().collect(Collectors.toMap(AdminUserAllDTO::getWeUserId, Function.identity(), (key1, key2) -> key2));
        }
        return sysUserMap;
    }

    @Override
    public AdminUserAllDTO findSysUserInfoByWeUserId(String weUserId) {
//        return this.baseMapper.findSysUserInfoByWeUserId(weUserId);
        return scrmSysUserClient.findSysUserByWeUserId(weUserId);
    }

    @Override
    public LoginUser findCurrentSysUserInfo(Long userId) {
//        return this.baseMapper.findCurrentSysUserInfo(userId);
        return ScrmSecurityUtils.getLoginUser();
    }
    ...
    public void updateCustomerUnionId(String unionId) {
        ...
        if (CollectionUtil.isEmpty(list)) {
            UnionidToExternalUserIdQuery query = new UnionidToExternalUserIdQuery(ScrmSecurityUtils.getCorpId(), ScrmSecurityUtils.getWxLoginUser().getOpenId(), unionId);
            UnionidToExternalUserIdVo externalUserIdVo = qwCustomerService.unionIdToExternalUserId3rd(query);
            if (externalUserIdVo != null && CollectionUtil.isNotEmpty(externalUserIdVo.getExternalUseridInfo())) {
                ...
                String externaklUserId = externalUseridInfo.stream().filter(item -> ObjectUtil.equal(ScrmSecurityUtils.getCorpId(), item.getCorpId())).map(UnionidToExternalUserIdVo.UnionIdToExternalUserIdList::getExternalUserId).findFirst().orElseGet(null);
                WeCustomer weCustomer = new WeCustomer();
