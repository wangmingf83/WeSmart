...
import cn.iocoder.yudao.module.smart.service.IWeContentViewRecordService;
...
import cn.iocoder.yudao.module.wecom.service.IQwMediaService;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.material.ao.*;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.query.media.WeGetMediaQuery;
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.query.media.WeMediaQuery;
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.vo.media.WeMediaVo;
import cn.iocoder.yudao.module.smart.handler.TextMaterialCellWriteHandler;
import cn.iocoder.yudao.module.smart.handler.TextMaterialImportDataListener;
...
//            }
            ...
            if (url != null) {
                return WeMaterialFileVo.builder().materialUrl(url).materialName(FileUtils.getName(url)).build();
            }
        } catch (Exception e) {
    ...
    public WeMediaVo uploadTemporaryMaterial(String url, String type, String name) {
        return iQwMediaService.upload(new WeMediaQuery(type, url, name));
    }

    @Override
    public WeMediaVo uploadWebhookMaterial(String key, String url, String type, String name) {
        return iQwMediaService.webhookUpload(new WeMediaQuery(key, type, url, name));
    }
        ...
        if (CollectionUtil.isNotEmpty(posterSubassemblyList)) {
            ...
            posterSubassemblyList.stream().forEach(wePosterSubassembly -> {
                ...
                if (wePosterSubassembly.getType().equals(1)) {
                    ...
                    if (Integer.valueOf(2).equals(fontTextAlign)) {//居中
                        textPrice.setCenter(true);
                    } else if (Integer.valueOf(1).equals(fontTextAlign)) { //左对齐
                        textPrice.setDirection(Direction.LeftRight);
                    } else if (Integer.valueOf(3).equals(fontTextAlign)) {//右对齐
...
//    }
    ...
    public WeMediaVo uploadImg(MultipartFile file) {
        return iQwMediaService.uploadImg(new WeMediaQuery(file));
    }
    ...
    public WeMediaVo uploadAttachmentMaterial(String url, String type, Integer attachmentType, String name) {
        ...
        WeMediaVo result = iQwMediaService.uploadAttachment(weMediaQuery);
        return result != null && null != result ? result : null;
    }
    ...
    public String mediaGet(String media_id, String fileType, String extentType) {
        try {
            byte[] data = FileUtils.inputTobyte(iQwMediaService.mediaGet(new WeMediaQuery(media_id)));
...
//    }
    ...
    public WeMediaVo getMediaToResponse(String mediaId) {
        try {
            WeMediaVo weMediaVo = iQwMediaService.getMedia(new WeGetMediaQuery(mediaId));
