...
public class WeAddMsgTemplateQuery {
    ...
    @Schema(description = "客户的外部联系人id列表，仅在chat_type为single时有效，不可与sender同时为空，最多可传入1万个客户")
    private List<String> external_userid;

    @Schema(description = "发送企业群发消息的成员userid，当类型为发送给客户群时必填")
    private String sender;

    @Schema(description = "消息文本内容，最多4000个字节")
    private Text text;

    @Schema(description = "附件，最多支持添加9个附件 ru")
    ...
    public static class Attachments {
        @Schema(description = "附件类型，可选image、link、miniprogram或者video")
