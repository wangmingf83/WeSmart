...
import cn.iocoder.yudao.module.common.utils.thread.WeMsgQiRuleThreadExecutor;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.qirule.vo.WeQiRuleUserVo;
...
public class WeChatMsgQiRuleWeeklyTask {
    ...
    private void calculateUserDataHandler(WeQiRuleScope weQiRuleScope, DateTime dateTime) {
        ...
        //客户会话数
        long chatNum = weChatContactMsgService.count(new LambdaQueryWrapper<WeChatContactMsg>()
        ...
        if(CollectionUtil.isNotEmpty(groupList)){
            long groupChatNum = weChatContactMsgService.count(new LambdaQueryWrapper<WeChatContactMsg>()
...
//                .apply("date_format(state_time, '%Y-%m-%d' ) = '" + dateTime.toDateStr() + "'"));
        LambdaQueryWrapper wrapper = new LambdaQueryWrapper<WeQiRuleUserStatistics>()
                .eq(WeQiRuleUserStatistics::getWeUserId, userId)
                .apply("date_format(state_time, '%Y-%m-%d' ) = '" + dateTime.toDateStr() + "'");
        WeQiRuleUserStatistics wrs = weQiRuleUserStatisticsService.getOne(wrapper);
        if(wrs == null) {
            weQiRuleUserStatisticsService.save(statistics);
        } else {
            weQiRuleUserStatisticsService.update(statistics, wrapper);
        }
    }
