...
import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeGroupMessageTemplateMapper;
...
public class WeGroupMessageTemplateServiceImpl extends ServiceImpl<WeGroupMessageTemplateMapper, WeGroupMessageTemplate> implements IWeGroupMessageTemplateService {
    ...
    public void addGroupMsgTemplate(WeAddGroupMessageQuery query) {
        ...
        if(query.getChatType().equals(Integer.valueOf(1))){ //发送客户
        ...
        }else if(query.getChatType().equals(Integer.valueOf(2))){ //发送客群


            if(CollectionUtil.isNotEmpty(senderList)){
                List<AdminUserAllDTO> sysUserVos = scrmSysUserClient.getUserListByWeUserIds(senderList.stream().map(WeAddGroupMessageQuery.SenderInfo::getUserId).collect(Collectors.toList()));
                        ...
                        WeGroupMessageTemplate.WeCustomersOrGroupQuery.builder()
                                .weGroupQuery(
                                        WeGroupMessageTemplate.WeGroupQuery.builder()
                                                ...
                                                .ownerNames(CollectionUtil.isNotEmpty(sysUserVos)?sysUserVos.stream().map(AdminUserAllDTO::getUsername).collect(Collectors.joining(",")):null)
                                                .build()
                                )
                                .build()
                );
        ...
        if (save(weGroupMessageTemplate)) {
            ...
            query.setCurrentUserInfo(ScrmSecurityUtils.getLoginUser());
            //保存附件
    ...
    public void syncGroupMsgSendResultByIds(List<Long> asList) {
        ...
        if (CollectionUtil.isNotEmpty(weGroupMessageLists)) {
            weGroupMessageLists.forEach(weGroupMessageList -> {
                ...
                WeGroupMsgListVo groupMsgTask = qwCustomerService.getGroupMsgTask(weGetGroupMsgListQuery);
                Optional.ofNullable(groupMsgTask).map(WeGroupMsgListVo::getTaskList).orElseGet(ArrayList::new).forEach(msgTask -> {
                    ...
                    WeGroupMsgListVo groupMsgSendResult = qwCustomerService.getGroupMsgSendResult(weGetGroupMsgListQuery);
