...
import cn.iocoder.yudao.module.common.enums.TrajectorySceneType;
import cn.iocoder.yudao.module.common.enums.TrajectoryType;
...
//        AjaxResult result = qwSysUserClient.getUserInfoById(weContentViewRecordQuery.getSendUserId());
        AdminUserAllDTO result = scrmSysUserClient.getUserInfoById(weContentViewRecordQuery.getSendUserId());
        //如果发送员工不存在，则跳过此次的记录
        if (result == null) {
            return;
        }
...
//        SysUser data = JSONObject.parseObject(s, SysUser.class);
        ...
        if (StringUtils
            ...
            List<WeCustomer> weCustomers = weCustomerMapper.selectList(new LambdaQueryWrapper<WeCustomer>()
                    .eq(WeCustomer::getAddUserId,result.getWeUserId())
        ...
        if (ObjectUtil.isNotEmpty(weCustomer)) {
            ...
            qwAppMsgBody.setCorpId(ScrmSecurityUtils.getCorpId());
            qwAppMsgBody.setCorpUserIds(Lists.newArrayList(result.getWeUserId()));
            ...
            //添加消息通知
            weMessageNotificationService.save(MessageTypeEnum.MATERIAL.getType(), result.getWeUserId(), MessageConstants.MATERIAL_LOOK, weCustomer.getCustomerName(), weMaterial.getMaterialName(), sb.toString());
        }

        //5.添加客户轨迹
//        if (ObjectUtil.isNotEmpty(weCustomer) && ObjectUtil.isNotEmpty(data)) {
        if (ObjectUtil.isNotEmpty(weCustomer) && result != null) {
            ...
            //被操作对象的id
            weCustomerTrajectory.setOperatoredObjectId(result.getWeUserId());
            //被操作对象名称
            weCustomerTrajectory.setOperatoredObjectName(result.getUsername());
            //客户id或群id，查询字段冗余,档该id不存在的时候代表
            weCustomerTrajectory.setExternalUseridOrChatid(weCustomer.getExternalUserid());
            //员工id，查询字段冗余
            weCustomerTrajectory.setWeUserId(result.getWeUserId());
            //动作
            ...
            weCustomerTrajectory.setContent(
                    String.format(TrajectorySceneType.TRAJECTORY_TITLE_LOOK_MATERIAL.getMsgTpl(), weCustomer.getCustomerName(), result.getUsername(), sb.toString())
            );
            //
