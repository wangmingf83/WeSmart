...
import cn.iocoder.yudao.module.common.utils.sql.SqlUtil;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.customer.vo.WeCustomerChannelCountVo;
...
import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeQrCodeMapper;
...
public class WeQrCodeServiceImpl extends ServiceImpl<WeQrCodeMapper, WeQrCode> implements IWeQrCodeService {
    ...
    public void addQrCode(WeQrAddQuery weQrAddQuery) {
        ...
        WeAddWayVo weAddWayResult = qwCustomerService.addContactWay(weContactWay);
    ...
    public void updateQrCode(WeQrAddQuery weQrAddQuery) {
        ...
        if(ObjectUtil.equal(qrCode.getType(),weQrAddQuery.getQrType())){
            WeAddWayQuery weContactWay = weQrAddQuery.getWeContactWay();
            WeResultVo weResultVo = qwCustomerService.updateContactWay(weContactWay);
            ...
            WeAddWayVo weResultVo = qwCustomerService.addContactWay(weContactWay);
            if(Objects.isNull(weResultVo)){
                throw new WeComException("活码生成失败！");
            }
            if (ObjectUtil.notEqual(0, weResultVo.getErrCode())) {
            ...
            ThreadUtil.execAsync(() ->{
                ...
                qwCustomerService.delContactWay(weContactWayQuery);
            });
        }
    ...
    public WeQrCodeDetailVo getQrDetail(Long qrId) {
        ...
        weQrCodeDetailVo.setQrShortLink(GuituAiConfig.getQrShortLinkDomainName() + encode);
    ...
    public PageInfo<WeQrCodeDetailVo> getQrCodeList(WeQrCodeListQuery qrCodeListQuery) {
        ...
        if (CollectionUtil.isNotEmpty(qrCodeIdList)) {
            ...
            for (WeQrCodeDetailVo qrCodeDetail: qrDetailByQrIds) {
                ...
                qrCodeDetail.setQrShortLink(GuituAiConfig.getQrShortLinkDomainName() + encode);
            }
...
//                });
    ...
    public WeAddWayVo createQrbyWeUserIds(List<String> weUserIds, String state) {
        return qwCustomerService.addContactWay(WeAddWayQuery.builder().type(2).scene(2).state(state).user(weUserIds).build());
    }

    @Override
    public void updateQrbyWeUserIds(List<String> weUserIds, String configId) {
        qwCustomerService.updateContactWay(WeAddWayQuery.builder().config_id(configId).user(weUserIds).build());
    }
    ...
    private void extracted(WeQrScopeVo weQrScope, String configId) {
        ...
        if(Objects.nonNull(corpAccount)){
            ...
            WeResultVo data = qwCustomerService.updateContactWay(weContactWay);
