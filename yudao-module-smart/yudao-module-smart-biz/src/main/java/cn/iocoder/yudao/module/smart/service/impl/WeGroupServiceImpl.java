...
import cn.iocoder.yudao.module.common.enums.GroupUpdateDetailEnum;
...
import cn.iocoder.yudao.module.smart.dal.dataobject.wecom.query.customer.groupchat.WeGroupChatDetailQuery;
...
import cn.iocoder.yudao.module.smart.dal.mysql.mapper.WeGroupMapper;
    ...
    public void synchWeGroupHandler(String msg) {
        ...
        if(CollectionUtil.isNotEmpty(chatIds)){
            chatIds.stream().forEach(chatId->{
                ...
                WeGroupChatDetailVo weGroupChatDetailVo = qwCustomerService.getGroupChatDetail(groupChatDetailQuery);
                ...
                if (weGroupChatDetailVo.getErrCode().equals(WeErrorCodeEnum.ERROR_CODE_0.getErrorCode())
                    ...
                    weGroup.setCreateBy(ScrmSecurityUtils.getUserName());
                    weGroup.setCreateById(ScrmSecurityUtils.getUserId());
                    weGroup.setUpdateBy(ScrmSecurityUtils.getUserName());
                    weGroup.setUpdateById(ScrmSecurityUtils.getUserId());
                    ...
                    if (CollectionUtil.isNotEmpty(memberLists)) {
                        memberLists.forEach(groupMember -> {
                            ...
                            weGroupMember.setCreateBy(ScrmSecurityUtils.getUserName());
                            weGroupMember.setCreateById(ScrmSecurityUtils.getUserId());
                            weGroupMember.setCreateTime(new Date());
                            weGroupMember.setUpdateBy(ScrmSecurityUtils.getUserName());
                            weGroupMember.setUpdateById(ScrmSecurityUtils.getUserId());
                            weGroupMember.setUpdateTime(new Date());
    ...
    public List<WeGroup> findGroupInfoFromWechat(WeGroupChatListQuery chatListQuery){
        ...
        if (CollectionUtil.isNotEmpty(groupChatList)) {
            ...
            for (WeGroupChatListVo.GroupChat groupChat : groupChatList) {
                ...
                WeGroupChatDetailVo result
                        = qwCustomerService.getGroupChatDetail(groupChatDetailQuery);
                if(null != result){
                    WeGroupChatDetailVo data = result;
...
//            }
    ...
    public void getGroupChatList( List<WeGroupChatListVo.GroupChat> groupChatList,WeGroupChatListQuery chatListQuery){
        ...
        WeGroupChatListVo groupChatListVo = qwCustomerService.getGroupChatList(chatListQuery);
    ...
    public void createWeGroup(String chatId, String corpId,boolean isCallBack) {
        ...
        WeGroupChatDetailVo chatDetailVo = qwCustomerService.getGroupChatDetail(query);
        ...
        if (groupChatDetail != null && groupChatDetail.getGroupChat() != null) {
            ...
            if (CollectionUtil.isNotEmpty(needQuitMemberList)) {
                needQuitMemberList.forEach(memeber -> {
                    ...
                    if(Integer.valueOf(0).equals(quitScene)){
                        //为被添加员工发送一条消息提醒
